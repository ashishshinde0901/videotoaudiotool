...
def download_youtube_video(link, save_dir):
    """Download a YouTube video or playlist and save it in the specified directory."""
    yt_dlp_path = get_bundled_path("yt-dlp.exe")
    if not os.path.exists(yt_dlp_path):
        raise FileNotFoundError(f"yt-dlp executable not found at {yt_dlp_path}")
    ...
    command = [
        yt_dlp_path,
        "-i",  # Continue downloading even if errors occur (for playlists)
        "-o", f"{save_dir}/%(title)s.%(ext)s",  # Save file using YouTube title
        "-f", "best",  # Best video quality
        link,
    ]
    try:
        subprocess.run(command, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        append_to_log(f"Video/Playlist downloaded successfully from {link}")
    except subprocess.CalledProcessError as e:
        raise RuntimeError(f"yt-dlp failed: {e.stderr.decode('utf-8')}")
    except Exception as e:
        raise RuntimeError(f"Unexpected error during video download: {e}")
...
def save_audio_files(file_paths, save_dir):
    """Save extracted audio files (vocals and background noise) into the selected folder."""
    try:
        for label, file_path in file_paths.items():
            target_path = Path(save_dir) / file_path.name
            shutil.move(str(file_path), target_path)
            append_to_log(f"{label} saved: {target_path}")
        messagebox.showinfo("Files Saved", f"Audio files saved in: {save_dir}")
    except Exception as e:
        append_to_log(f"Error saving files: {e}")
...
class RianVideoProcessingTool(ctk.CTk):
    ...
    def process_youtube_video(self, youtube_link_var, progress_label):
        """Download and process a YouTube video or playlist."""
        link = youtube_link_var.get().strip()
        if not link:
            progress_label.set("YouTube link is empty.")
            append_to_log("YouTube link is empty.")
        ...
        if not save_dir:
            progress_label.set("No folder selected.")
            append_to_log("No folder selected for saving videos.")
            return

        progress_label.set("Downloading video/playlist... Please wait.")
        start_time = datetime.now()
        try:
            download_youtube_video(link, save_dir)
            end_time = datetime.now()
            ...
            send_log_to_server(log_data)
            progress_label.set(f"Download complete. Files saved in: {save_dir}")
        except Exception as e:
            progress_label.set(f"Download failed: {e}")
            append_to_log(f"Error downloading video/playlist: {e}")
    ...
    def init_local_processing(self):
        ...
        ctk.CTkButton(
            self.content_frame,
            text="Upload File",
            command=lambda: run_in_thread(self.process_local_video, progress_label),
        ).pack(pady=20)
        ctk.CTkLabel(self.content_frame, textvariable=progress_label, font=("Helvetica", 14)).pack(pady=10)

    def process_local_video(self, progress_label):
        ...
        save_dir = filedialog.askdirectory(title="Select Folder to Save Audio Files")
        if not save_dir:
            progress_label.set("No folder selected.")
            append_to_log("No folder selected for saving audio files.")
            return

        progress_label.set("Processing video... Please wait.")
        start_time = datetime.now()
        try:
            vocals_path, noise_path, _ = process_video(file_path, save_dir)
            end_time = datetime.now()

            save_audio_files(
                {
                    "Vocals": vocals_path.rename(Path(save_dir) / f"clean-{Path(file_path).stem}.wav"),
                    "Background Noise": noise_path.rename(Path(save_dir) / f"bg-{Path(file_path).stem}.wav"),
                },
                save_dir
            )
            log_data = {
            ...
            send_log_to_server(log_data)
            progress_label.set(f"Processing complete. Files saved in: {save_dir}")
        except Exception as e:
            progress_label.set(f"Processing failed: {e}")
            append_to_log(f"Error processing video: {e}")
